...
Signals used:
  ...
  - social   : inferred (Pinterest/TikTok NOT pulled this round — flagged in notes)
...
IDEAS = [
    ("GPD01", "ceramic mug", "grandma", "grandkids names", "sentimental/milestone",
     "Grandma + grandkids' names (Promoted to Grandma)", 2200, "rising", "46 in-cart (promoted-to-grandma)", "med",
     [5, 4, 3, 3, 5, 4, 5, 5]),
    ("GPD02", "sweatshirt", "grandma", "grandkids names + birth flowers", "identity/sentimental",
     "Grandma's Garden — a birth flower per grandchild", 250, "rising", "64 in-cart (floral applique swtsht)", "high (birth-flower)",
     [4, 5, 5, 3, 5, 3, 5, 5]),
    ("GPD03", "necklace (jewelry)", "grandma", "grandkids birthstones/names", "sentimental",
     "A birthstone for every grandchild", 350, "rising", "291 in-cart (name necklace)", "high",
     [3, 5, 4, 3, 5, 4, 5, 4]),
    ("GPD04", "stainless tumbler", "grandma", "grandkids names + birth flower", "identity",
     "Nana's Garden tumbler", 600, "rising", "birth-flower tumbler bestsellers", "high (birth-flower)",
     [4, 3, 4, 3, 4, 4, 5, 5]),
    ("GPD05", "fleece/woven blanket", "grandma", "grandkids names, floral", "sentimental",
     "Grandma's woven family blanket", 500, "steady", "24 in-cart (woven blanket)", "med",
     [3, 3, 3, 4, 5, 3, 5, 5]),
    ("GPD06", "t-shirt", "grandma", "nickname + grandkids names", "identity",
     "In a world full of grandmas, be a Mimi (Gigi/Nana/Mimi)", 500, "steady", "63 in-cart; shops w/100k+ sales", "med",
     [4, 5, 4, 2, 4, 3, 4, 5]),
    ("GPD07", "ceramic mug", "grandpa", "grandkids names", "milestone",
     "First Dad, Now Grandpa", 4900, "rising", "178 in-cart (bestseller)", "med",
     [3, 5, 3, 3, 5, 4, 5, 5]),
    ("GPD08", "apron", "grandpa", "grandkids names", "identity/funny",
     "Grandpa's Grilling Crew + kids' names", 4900, "steady", "793 in-cart (grill cutting board); 82 grill plate", "med",
     [3, 5, 3, 3, 4, 4, 5, 4]),
    ("GPD09", "t-shirt", "grandpa", "photo", "sentimental",
     "Best Papa Ever + photo", 450, "rising", "290 in-cart (photo grandpa shirt)", "med",
     [3, 5, 3, 3, 4, 3, 5, 4]),
    ("GPD10", "t-shirt", "grandma", "none/text", "funny",
     "You can't tell me what to do, you're not my grandkids", 500, "steady", "25-31 in-cart (funny tees)", "high (shareable)",
     [3, 4, 4, 2, 3, 3, 4, 4]),
    ("GPD11", "doormat", "both", "nicknames", "sentimental/home",
     "Life is better at Gigi & Poppy's house", 200, "steady", "proven cat: shop w/720k sales", "med",
     [2, 3, 3, 3, 4, 3, 5, 5]),
    ("GPD12", "canvas/wood sign", "grandma", "photo + grandkids names", "sentimental",
     "Grandkids make life grand", 200, "steady", "photo-holder bestseller", "med",
     [2, 4, 3, 3, 4, 3, 5, 5]),
    ("GPD13", "pillow", "grandma", "birth flowers + grandkids names", "sentimental",
     "Grandma's Garden — where love grows", 250, "rising", "birth-flower pillow bestseller", "high (birth-flower)",
     [3, 3, 5, 3, 4, 4, 5, 5]),
    ("GPD14", "cap/hat", "grandpa", "est. year", "milestone",
     "Grandpa Est. 2026", 450, "rising", "74 in-cart (EST hat)", "med",
     [3, 4, 3, 3, 4, 4, 5, 4]),
]
...
for (iid, prod, rec, pers, angle, hook, vol, slope, mk, soc, v) in IDEAS:
    ...
    rows.append([f"IDEA-{iid}", DATE, status, prod, rec, OCC, pers, angle, hook,
                 "ahrefs+etsy", vol, slope, mk, soc, demand,
                 v[3], v[4], v[5], v[6], v[7], comp, "", "", "", "", "", "",
                 "", "social(pinterest/tiktok) not pulled this round", ""])
