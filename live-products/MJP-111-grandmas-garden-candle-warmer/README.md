...
**Working folder for ad production.** Status overview lives in `product-tracker/` (row MJP-111).
...
Product photos live on the Shopify CDN (2048×2048 webp). Save local copies into `source-images/`. For the video models (which need jpg/png), convert on the fly via the weserv proxy: `https://images.weserv.nl/?url=ssl:<cdn-url-without-https>&output=jpg&w=1080`.
...
> ⚠️ These store images are generic/mockup shots. **Preferred source = the finished product image with the personalized names printed on it** (user to provide) → put it in `source-images/`.

## Clips (`clips/`) & final (`final/`)
...
- Production clips → generate with Kie.ai **Kling 3.0 Turbo** (`kling_v3_turbo_video`), save to `clips/`.
- Assembled ad → HyperFrames render, save to `final/`.
